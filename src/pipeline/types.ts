...
export interface MetricEntryInput {
  ...
  /**
   * Free-form key/value tags attached to this entry.
   * Only meaningful at root level — nested metric entries created via field
   * references don't carry their own tags. Persisted into entry_tags.
   */
...
export interface ResolvedEntry {
  ...
  /** True if this entry was resolved from an existing DB entry (not newly created) */
  resolvedFromExisting?: boolean;
  /** Tags carried through from the root MetricEntryInput; persisted after batch insert. */
  tags?: Map<string, string>;
}
