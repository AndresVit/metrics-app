...
function applyScalarOp(left: number, op: string, right: number): PipelineResult<number | null> {
  switch (op) {
    case '+':
    ...
    case '/':
      if (right === 0) {
        // Return null for division by zero — the AttributeEntry is still created
        // with a null value (not skipped). This satisfies cardinality 1..1 while
        // signaling "cannot compute" (e.g., productivity ratios when no productive time was logged).
        return createSuccess(null);
      }
      return createSuccess(left / right);
    case '//':
      if (right === 0) {
        // Return null for integer division by zero
        return createSuccess(null);
      }
      return createSuccess(Math.floor(left / right));
    case '%':
      if (right === 0) {
        // Return null for modulo by zero
        return createSuccess(null);
      }
      return createSuccess(left % right);
    case '^':
