...
function processEntry(
  ...
  for (const field of sortedFields) {
    ...
    // IMPORTANT: null values vs missing fields
    // - null value: Formula evaluated successfully but result is null (e.g., division by zero)
    //   We MUST still create the AttributeEntry with a null value.
    //   This satisfies cardinality (1..1 field has 1 instance, just with null value).
    // - Missing field: No value provided and no formula. No AttributeEntry is created.
    //   This violates cardinality (1..1 field has 0 instances → error).
    //
    // Widgets render null as "—" which is the desired UX for "cannot compute".
    ...
    if (existingEntry && existingEntry.attributeEntry) {
      // Only set value if not null; null keeps the AttributeEntry with all-null typed columns
      if (value !== null) {
        setAttributeValue(
      ...
      // AttributeEntry is initialized with all null typed columns.
      // We only call setAttributeValue if value is not null.
      // A null-valued AttributeEntry still satisfies cardinality - it EXISTS but has no value.
  ...
  for (const child of entry.children) {
    // Skip entries resolved from existing DB entries — their formulas
    // were already computed when originally inserted.
