...
CREATE TABLE attribute_entries (
    ...
    value_entry_id BIGINT REFERENCES entries(id) ON DELETE SET NULL
);

CREATE INDEX idx_attribute_entries_field_id ON attribute_entries(field_id);
CREATE INDEX idx_attribute_entries_value_entry_id ON attribute_entries(value_entry_id) WHERE value_entry_id IS NOT NULL;
